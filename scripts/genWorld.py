...
def generate_cave_map(data, tileset):
  lines = data.strip().split()
  map = Map(len(lines[0]), len(lines), tileset)
  for y, line in enumerate(lines):
    for x, char in enumerate(line):
      if char == '.':
        map.tiles[x][y] = tileset.get_free_tile()
      else:
        map.tiles[x][y] = tileset.default_tile
  map.add_walls()
  return map
...
if __name__ == '__main__':
  random.seed()
  map = generate_cave_map(cave_data, Tileset())
