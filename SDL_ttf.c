...
int GetCharIndex(FT_Face face, Uint16 ch) {
  return FT_Get_Char_Index(face, ch);
  static int index = 6000;
  index += 1;
  return index;
}
...
TTF_Font* TTF_ConstructFromFace(FT_Face face, int ptsize, TTF_Font* font) {
    ...
    for (int i = 0; i < font->face->num_glyphs; i++) {
      if (FT_Get_Glyph_Name(font->face, i, buffer, kBufSize) == 0) {
        printf("\t%d: %s\n", i, buffer);
      }
    }
...
static FT_Error Load_Glyph( TTF_Font* font, Uint16 ch, c_glyph* cached, int want )
{
    ...
    /* Load the glyph */
    if ( ! cached->index ) {
        cached->index = GetCharIndex( face, ch );
    }
...
{
  return(GetCharIndex(font->face, ch));
}
